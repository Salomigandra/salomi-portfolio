...
The pass-through chain is four multiplications, each with a stated source:

1. **Crude → landed cost**: Brent price × USD/INR rate × import dependency (~90%, PPAC). Both shock variables move together — that's why this crisis compounds: a 51% crude rise landed as a ~68% rupee-cost rise.
2. **Landed cost → pump price**: pump prices are administered, so I model a pass-through *scenario* (₹14/L) rather than a mechanical formula, benchmarked against OMC under-recovery disclosures (~₹1,700 crore/day).
3. **Pump price → household cost**: vehicle type × km/month × mileage. The calculator exposes these as user inputs instead of hiding averages.
4. **Burden by income**: fuel + cooking-energy share of household spending by consumption quintile (NSSO/HCES). The bottom quintile spends roughly 2.6× the share of its budget on energy that the top quintile does — same price shock, very different pain. That's the headline number, and it's a *relative burden* ratio, not a rupee ratio.

**Limits.** Administered pricing means step 2 is a scenario, not a forecast — the government can absorb the shock fiscally, as it partially did. FPI outflow and reserve figures are point-in-time estimates that were being revised weekly. The food-CPI channel (fertiliser → crop cost) is directionally modeled, not econometrically estimated.
...
Two things in this data matter more than the headline crude price. First, the **quintile asymmetry**: any fiscal response designed around average household impact will underserve the bottom 40%, because their energy burden is structurally 2.6× heavier — targeted LPG subsidy protection beats broad excise cuts on equity per rupee. Second, of PM Modi's four asks, the data ranks **gold-import reduction** as the highest-leverage behavioural lever ($3–4B forex saved per 10% reduction, versus ~$1B for travel cuts) — yet it received the least public attention. When the levers are quantified side by side, the communication priority looks misallocated.
