...
/** Native + roman (roman shows only if different) */
const HELLOS = [
...
export default function AboutHero() {
  ...
  return (
    <section className={`relative overflow-hidden ${styles.pastelHero}`}>
      <div className="mx-auto flex min-h-[80vh] max-w-5xl flex-col items-center justify-center px-6 text-center">
        {/* Rotating hello: emoji + native + (roman if different) + language pill */}
        <div className="relative h-28 md:h-32 lg:h-36 w-full select-none" aria-live="polite" aria-atomic="true">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              {/* native big + roman in parentheses (only if different) */}
              <div className="flex items-baseline gap-2">
                <bdi dir="auto" className="text-4xl md:text-6xl lg:text-7xl font-semibold tracking-tight">
                  {item.text}
                </bdi>
                {showRoman && (
                  <span className="text-2xl md:text-4xl lg:text-5xl font-normal text-black/60">
                    ({item.roman})
                  </span>
                )}
              </div>
        ...
        {/* Name + subline */}
        <h1 className="mt-4 text-6xl md:text-8xl font-bold tracking-tight">
          I’m{" "}
...
/* ---------- helper components (outside the JSX above) ---------- */

function WavesSoft() {
  return (
    <div className="pointer-events-none absolute bottom-0 left-0 right-0">
      {/* Soft, taller container */}
...
/** Single scrolling wave band (no external assets) */
